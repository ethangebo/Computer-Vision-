...
class Mapper:
    ...
    def __init__(
        ...
        self._last_camera_facing = None

    def reset_plane(self):
        """Clear cached plane. Call when switching between front/rear camera."""
        self.plane = None
        self._last_camera_facing = None
    ...
    def update_from_depth(self, depth_m, fx, fy, cx, cy, camera_facing="front"):
        """
        camera_facing: "front" or "rear". For rear camera, rover x_forward = -Z, y_left = -X
        (camera Z points backward; lateral convention same as front).
        """
        # Reset plane when switching cameras (front/rear see different ground)
        if camera_facing != self._last_camera_facing:
            self.plane = None
            self._last_camera_facing = camera_facing
        self._frame_i += 1
        H, W = depth_m.shape
        ...
        if Po.shape[0] > 0:
            if camera_facing == "rear":
                xy_b = np.stack([-Po[:, 2], -Po[:, 0]], axis=1).astype(np.float32)  # x_forward=-Z, y_left=-X
            else:
