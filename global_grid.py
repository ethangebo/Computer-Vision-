...
class GlobalOccupancyGrid2D:
    ...
    def mark_occupied_region(self, center_x_m: float, center_y_m: float, radius_m: float) -> int:
        """
        Mark all cells whose center lies within radius_m of (center_x_m, center_y_m) as occupied.
        Use for static arena obstacles (e.g. KSC column). Returns number of cells marked.
        """
        r0, c0 = self.world_to_grid(center_x_m, center_y_m)
        rad_cells = max(1, int(np.ceil(radius_m / self.res)))
        n = 0
        for dr in range(-rad_cells, rad_cells + 1):
            for dc in range(-rad_cells, rad_cells + 1):
                ...
                if not self.in_bounds(r, c):
                    continue
                # Cell center in world (same convention as grid_to_world)
                cell_x = (c - self.cx + 0.5) * self.res
                cell_y = (self.cy - r - 0.5) * self.res
                if (cell_x - center_x_m) ** 2 + (cell_y - center_y_m) ** 2 <= radius_m ** 2:
                    if self.add_occupancy_at(r, c):
                        n += 1
        return n
